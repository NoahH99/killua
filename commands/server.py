...
# Global lock to prevent concurrent start/stop/restart operations
...
def create_server_commands(
    ...
    async def mc_start(interaction: discord.Interaction, debug: bool = False):
        ...
        if operation_lock.locked():
            embed = discord.Embed(
                title="Start Server",
                description="Another server operation is currently in progress. Please wait for it to complete.",
    ...
    async def mc_stop(interaction: discord.Interaction, debug: bool = False):
        ...
        # Acquire lock for the stop operation
        ...
        try:
            ...
            logger.error(f"Unexpected error in mc_stop: {e}")
            operation_lock.release()
            raise
